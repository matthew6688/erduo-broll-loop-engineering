...
5. For digital-presenter work, decide presenter versus B-roll from the viewed chapter result; Parent may compile the final decisions but must not invent or rewrite them.
...
10. Treat the standard command's aggregated Assignment Preflight and rendered text/motion revision reports as one bounded build loop: fix every reported preflight issue together, then use at most two actual render attempts; a third render is forbidden and returns to the Recipe or Runtime Plan.
